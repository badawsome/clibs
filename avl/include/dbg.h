//
// Created by badcw on 2022/3/24.
//

#ifndef CLIBS_AVL_INCLUDE_DBG_H
#define CLIBS_AVL_INCLUDE_DBG_H
...
#define VERIFY(cond) (void)((!(cond)) && libspl_assert(#cond, __FILE__, __FUNCTION__, __LINE__))
#define verify(cond) (void)((!(cond)) && libspl_assert(#cond, __FILE__, __FUNCTION__, __LINE__))
...
#define VERIFY3B(LEFT, OP, RIGHT)                                                                                      \
    do {                                                                                                               \
        ...
        if (!(__left OP __right))                                                                                      \
            libspl_assertf(__FILE__, __FUNCTION__, __LINE__, "%s %s %s (0x%llx %s 0x%llx)", #LEFT, #OP, #RIGHT,        \
...
#define VERIFY0(LEFT)                                                                                                  \
    do {                                                                                                               \
        const uint64_t __left = (uint64_t)(LEFT);                                                                      \
        if (!(__left == 0))                                                                                            \
            libspl_assertf(__FILE__, __FUNCTION__, __LINE__, "%s == 0 (0x%llx == 0)", #LEFT, (u_longlong_t)__left);    \
    } while (0)
...
#define ASSERTV(x) x
#define IMPLY(A, B)                                                                                                    \
    ((void)(((!(A)) || (B)) || libspl_assert("(" #A ") implies (" #B ")", __FILE__, __FUNCTION__, __LINE__)))
#define EQUIV(A, B)                                                                                                    \
